...
| emoji_generator | function | pymdownx.emoji.to_png | A function that takes the emoji info and constructs the desired emoji ouput. This is used to select between the default, provided generators, or to specify your own. See [Default Emoji Generators](#default-emoji-genrators) to see the provided generators. |
| title | str | 'short' | Specifies the title format that is fed into the emoji generator function.  Can either be `long` which is the long description of the Unicode emoji, `short` which is the short name (`:short:`), or `none` which will simply pass `None` to the generator. |
| alt | str | 'unicode' | Specifies the format for the alt value that is passed to the emoji generator function.  The alt value is often used as the fallback value for emoji images.  If `alt` is set to `short`, the short name will be passed to the generator.  If `alt` is set to `unicode` the Unicode characters are passed to the generator.  Lastly, if `alt` is set to `html_entity`, the Unicode characters encoded as an HTML entities will be passed to the generator. |
...
!!! tip "Legacy GithubEmoji Emulation"
    The Emoji extension was actually created to replace the now deprecated [githubemoji](./githubemoji.md) extension. Emoji was written to be much more flexible.  If have a desire to configure the output to be identical to the legacy githubemoji extension, you can use the settings below. To learn more about settings, read on through the documentation.
...
: 
    This provides an indexing using the latest Gemoji supported emoji (at the time of release). You can always find out what tag it was built with by doing the following:
...
: 
    This generator supports written for output of EmojiOne.  It is expected that this will be used in conjunction with the the official EmojiOne CSS.  You can include the CSS from the CDN in your document (version 2.2.7 at the time of writing this): https://cdnjs.cloudflare.com/ajax/libs/emojione/2.2.7/assets/sprites/emojione.sprites.css.  This outputs the emoji as a PNG image in the form of:
...
: 
    ...
    | shortname | str | The current short name name.  This may differ from the actual name used in a document as alias get translated to the main short name. |
    | alias | str | If the name specified in the document was an alias, that name will be found here, and the main short name will be found under `shortname`.  This will be `None` if the name used was not an alias. |
    | uc | str | This is a friendly name for the Unicode value.  This name is used to reference emoji image names or for specifying class names etc.  The friendly name consists of each Unicode value represented as a hex value separated with hyphens.  Values such as U+200D ZERO WIDTH JOINER and  U+FE0F VARIATION SELECTOR-16 are stripped out.  So the value here will not always be practical for calculating the actual Unicode points of an emoji.  Gemoji has some non-standard emoji that are not Unicode; this will be `None` if the emoji is not Unicode. |
    | alt | str | This is the alternative emoji value (or fallback value).  Its format will differ depending on the extension setting `alt`.  If `alt` is `short`, this will simply be the short name used in the document.  If `alt` is `unicode`, this will return the actual Unicode characters for the emoji.  if `alt` is `html_entity`, this will be Unicode characters encoded in HTML entity format.  Alt can be inserted directly into an HTML document to render the actual Unicode character if desired. |
    | title | str  | This is the title that can be used in img elements.  If the extension setting `title` is set to `short`, this will be the short name used. If the extension setting `title` is set to `long`, this will be the long emoji name specified in the index.  If the extension setting `title` is set to `none`, this will be passed to the generator function as `None`. |
...
6. Force test update `python run_tests.py -fu`.
...
```
EmojiOne :smile: emojis are very useful :thunbsup:.
```
